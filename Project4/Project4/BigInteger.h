...
public:
    ...
    //計算a的n次方
    static void myPow(BigInteger& a, int n);
    //2、8進制字串構造大整數( 注：16進制用另一個函數處理 )
    static BigInteger BuildBigIntFormBinOrOct(string str, int i);
    static BigInteger BuildBigIntFormHex(string str);
    //轉換為2進制字串
    string toBinString();
    //轉換為8進制字串
    string toOctString();
    ...
    string toHexString();
    // 2、8、16進制：加、減、乘法運算
    static string BaseOperation(string num1, string num2, int base, char oper);
